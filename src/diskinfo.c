...
void displayDiskImage(char* os, char* label, int size, int free, int num_of_files, int fat, int sectors);
...
int main(int argc, char* argv[]) {
  ...
  int num_of_files = fileCount(address + SECTOR_SIZE * 19);
  int size = sizeOfDisk(address);
  int free = freeDisk(address);

  displayDiskImage(os, label, size, free, num_of_files, fat, sectors);
  ...
  while(address[0] != 0x00) {
    if(address[0] != '.' && address[1] != '.' && address[26] != 0x00 && address[26] != 0x01 && address[11] != 0x0f && (address[11] & 0x08) != 0x08) {
      if((address[11] & 0x10) != 0x10) {
        ...
        cnt += fileCount(load + (address[26] + 12) * SECTOR_SIZE);
      }
    }
    address += 0x20;
  }
